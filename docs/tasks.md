...
> 最終更新: 2026-02-28（productName 修正・拡張子変更）
> 凡例: ✅ 完了 / ⚠️ 部分実装 / ⬜ 未着手
...
| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 8-1 | ID カウンター衝突バグ修正（ドリルダウン後に誤ったユニットが表示される） | ✅ | `syncIdCounter()` を `mandalaHelpers.ts` に追加し、`initFromFile` で呼び出す |
| 8-2 | ファイル関連付け起動（`.mandala` をダブルクリックでアプリ起動＋ファイルを開く） | ✅ | Rust `get_startup_file` コマンド（CLI 引数解析）＋ 起動時 `useEffect` で優先読み込み |
| 8-3 | `productName` 修正（スタートメニュー表示名を「Mandala-ya」に） | ✅ | `tauri.conf.json` の `productName` を `"Mandalaya"` → `"Mandala-ya"` |
| 8-4 | 保存ファイル拡張子を `.mandala` → `.mandalaya` に変更 | ✅ | Tauri設定(Win/Mac)・TS・Rust 全箇所を統一変更 |
| 8-5 | 俯瞰表示のサイズをユニット表示と同じ比率に変更 | ✅ | 700px上限を撤廃し、高さ→幅の6:4比率変換を適用 |
